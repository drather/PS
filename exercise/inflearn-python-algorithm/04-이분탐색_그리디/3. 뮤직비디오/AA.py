...
def check(capacity, count, songs):
    start_idx = 0
    end_idx = start_idx
    sub_total = 0
    total = 0

    while end_idx < len(songs):
        if count <= 0:
            return False

        if sub_total + songs[end_idx] <= capacity:
            sub_total += songs[end_idx]
            end_idx += 1
...
song_count, dvd_count = map(int, input().split())
songs = list(map(int, input().split()))

# song_count, dvd_count = 9, 3 ; arr = [1, 2, 3, 4, 5, 6, 7, 8, 9]
