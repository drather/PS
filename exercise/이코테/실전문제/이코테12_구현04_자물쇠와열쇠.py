...
열쇠를 나타내는 2차원 배열 key 와 자물쇠를 나타내는 2차원 배열 lock 이 매개변수로 주어질 때,
열쇠로 자물쇠를 열수 있으면 true 를, 열 수 없으면 false 를 return 하도록 solution 함수를 완성해주세요.

제한사항
key 는 M x M(3 ≤ M ≤ 20, M은 자연수)크기 2차원 배열입니다.
lock 은 N x N(3 ≤ N ≤ 20, N은 자연수)크기 2차원 배열입니다.
M은 항상 N 이하입니다.
key 와 lock 의 원소는 0 또는 1로 이루어져 있습니다.
...
SOLUTION
- 구현 문제이므로, 여러 조건들을 만족하게끔 구현한 뒤, 알맞게 실행한다.
- 구현할 것
1. 주어진 Key 를 시계방향으로 회전하는 메서드
2. 상하좌우로 이동하는 메서드
    - 상
    - 하
    - 좌
